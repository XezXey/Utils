...
def optical_flow_estimation(images, output_path='./', normalize=True):
  ...
  for i in tqdm(range(len(images)-1)):
    if normalize:
      im1 = images[i].astype(float) / 255.
      im2 = images[i+1].astype(float) /255.
    else:
      im1 = images[i]
      im2 = images[i+1]
    ...
    flows_forward.append(np.concatenate((u[..., None], v[..., None]), axis=2))
    # Create a Forward frame
    hsv = np.zeros(im1.shape, dtype=np.uint8)
    hsv[:, :, 0] = 255
    hsv[:, :, 1] = 255
    mag, ang = cv2.cartToPolar(flows_forward[i][..., 0], flows_forward[i][..., 1])
    hsv[..., 0] = ang * 180 / np.pi / 2
    hsv[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
    rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
    cv2.imwrite(output_optical_flow_path_fw + 'outflow_frame{}.png'.format(i), rgb)
    ...
    u, v, im2W = pyflow.coarse2fine_flow(
      ...
      nSORIterations, colType)
    flows_backward.append(np.concatenate((u[..., None], v[..., None]), axis=2))
    # Create a Backward frame
    hsv = np.zeros(im1.shape, dtype=np.uint8)
    hsv[:, :, 0] = 255
    hsv[:, :, 1] = 255
    mag, ang = cv2.cartToPolar(flows_backward[i][..., 0], flows_backward[i][..., 1])
    hsv[..., 0] = ang * 180 / np.pi / 2
    hsv[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
    rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
    cv2.imwrite(output_optical_flow_path_bw + 'outflow_frame{}.png'.format(i), rgb)
    cv2.imwrite(output_optical_flow_path_bw + 'outflow_warped_frame{}.png'.format(i), im2W[:, :, ::-1] * 255)

  # Cast to np array-like              
